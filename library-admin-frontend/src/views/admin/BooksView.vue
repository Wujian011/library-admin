<template>
  <div class="p-6 max-w-[1400px] mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 tracking-tight">图书管理</h1>
        <p class="text-gray-500 text-sm mt-1">管理系统内的所有图书信息</p>
      </div>
      <button
        @click="handleAdd"
      ...
      <div class="p-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex flex-wrap gap-3 items-center">
        <div class="relative group">
          ...
          <input
            ...
            placeholder="搜索书名..."
            class="pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-lg w-48 focus:outline-none focus:ring-2 focus:ring-[#07C36E]/20 focus:border-[#07C36E] transition-all text-sm"
          />
        </div>
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">书名</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">作者</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">出版社</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">库存</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">价格</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr v-else v-for="row in tableData" :key="row.bookId" class="hover:bg-gray-50/80 transition-colors group">
              <td class="px-6 py-4 text-gray-500 text-sm font-mono">#{{ row.bookId }}</td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-600 font-bold text-sm border border-white shadow-sm">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                  </div>
                  <div>
                    <div class="font-medium text-gray-900 max-w-[200px] truncate" :title="row.name">{{ row.name || '未命名' }}</div>
                    <div v-if="row.isbn" class="text-xs text-gray-400">ISBN: {{ row.isbn }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 text-gray-600 text-sm">{{ row.author || '-' }}</td>
              <td class="px-6 py-4 text-gray-600 text-sm max-w-[150px] truncate" :title="row.publisher">{{ row.publisher || '-' }}</td>
              <td class="px-6 py-4">
                <div class="text-sm">
                  <span class="text-gray-900 font-medium">{{ row.availableQty }}</span>
                  <span class="text-gray-400"> / {{ row.totalQty }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-gray-600 text-sm">{{ row.price ? `¥${row.price}` : '-' }}</td>
              <td class="px-6 py-4">
                <span
                  :class="row.status === 1
    ...
    >
      <div v-if="modalOpen" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50" @click.self="modalOpen = false">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all scale-100">
          <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50">
            <h2 class="text-lg font-bold text-gray-800">{{ isEdit ? '编辑图书' : '添加图书' }}</h2>
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">书名 <span class="text-red-500">*</span></label>
              <input
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">出版社</label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">出版日期</label>
                <input
            ...
            <div class="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select v-model="formData.classId" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#07C36E]/20 focus:border-[#07C36E] appearance-none transition-all text-sm">
            ...
            <div class="grid grid-cols-3 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">藏书总数 <span class="text-red-500">*</span></label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">可借数量 <span class="text-red-500">*</span></label>
                <input
                  v-model="formData.availableQty"
                  type="number"
                  min="0"
                  class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#07C36E]/20 focus:border-[#07C36E] transition-all text-sm"
                  required
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">上架状态 <span class="text-red-500">*</span></label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">图书简介</label>
              <textarea
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
          <div class="p-6">
            <div class="flex items-center gap-4 mb-4">
              ...
              <div class="flex-1">
                <h3 class="text-lg font-bold text-gray-900 mb-1">确认删除图书</h3>
            ...
            <div v-if="deleteTarget" class="bg-gray-50 rounded-lg p-4 mb-6">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-600 border border-white shadow-sm">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                </div>
                <div>
                  <div class="font-medium text-gray-900">{{ deleteTarget.name || '未命名' }}</div>
                  <div class="text-sm text-gray-500">{{ deleteTarget.author || '未知作者' }}</div>
                </div>
              </div>
            </div>
...
import { listBookInfoByPage, deleteBookInfo, addBookInfo, updateBookInfo } from '@/api/bookinfo'
import { listClassInfoByPage } from '@/api/classinfo'
...
const loadClassList = async () => {
  try {
    const res = await listClassInfoByPage({ current: 1, pageSize: 1000 })
    classList.value = res.data?.records || []
    if (classList.value.length === 0) {
      console.warn('分类列表为空，请先添加图书分类')
    }
  } catch (error: any) {
    console.error('加载分类列表失败', error)
    toast.add({ title: '加载分类列表失败', description: error.message, color: 'red' })
  }
}
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await listBookInfoByPage({
...
const handleEdit = (row: any) => {
  ...
  formData.pubTime = row.pubTime || ''
  formData.language = row.language || ''
  formData.price = row.price
  formData.totalQty = row.totalQty || 0
  formData.availableQty = row.availableQty || 0
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 处理空字符串字段
    if (!submitData.isbn || submitData.isbn.trim() === '') {
      submitData.isbn = null
    }
    if (!submitData.pubTime || submitData.pubTime.trim() === '') {
      submitData.pubTime = null
    }

    if (isEdit.value) {
      await updateBookInfo(submitData)
    } else {
      await addBookInfo(submitData)
    }
...
const confirmDelete = async () => {
  ...
  deleting.value = true
  try {
    await deleteBookInfo(deleteTarget.value.bookId as number)
