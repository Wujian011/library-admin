<template>
  <div class="p-6 max-w-[1400px] mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 tracking-tight">图书分类管理</h1>
        <p class="text-gray-500 text-sm mt-1">管理系统内的所有图书分类</p>
      </div>
      <button
        @click="handleAdd"
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
            <tr>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">分类ID</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">分类名称</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">创建时间</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">更新时间</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr v-else v-for="row in tableData" :key="row.classId" class="hover:bg-gray-50/80 transition-colors group">
              <td class="px-6 py-4 text-gray-500 text-sm font-mono">#{{ row.classId }}</td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-600 font-bold text-sm border border-white shadow-sm">
                    {{ row.className ? row.className.charAt(0).toUpperCase() : 'C' }}
                  </div>
                  <div class="font-medium text-gray-900">{{ row.className || '未命名' }}</div>
                </div>
              </td>
              <td class="px-6 py-4 text-gray-500 text-sm whitespace-nowrap">{{ formatDate(row.createTime) }}</td>
              <td class="px-6 py-4 text-gray-500 text-sm whitespace-nowrap">{{ formatDate(row.updateTime) }}</td>
      ...
      <div v-if="modalOpen" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50" @click.self="modalOpen = false">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
          <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50">
            <h2 class="text-lg font-bold text-gray-800">{{ isEdit ? '编辑分类' : '添加分类' }}</h2>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
          <div class="p-6">
            <div class="flex items-center gap-4 mb-4">
              ...
              <div class="flex-1">
                <h3 class="text-lg font-bold text-gray-900 mb-1">确认删除分类</h3>
            ...
            <div v-if="deleteTarget" class="bg-gray-50 rounded-lg p-4 mb-6">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center text-blue-600 font-bold text-sm border border-white shadow-sm">
                  {{ deleteTarget.className ? deleteTarget.className.charAt(0).toUpperCase() : 'C' }}
                </div>
                <div>
                  <div class="font-medium text-gray-900">{{ deleteTarget.className || '未命名' }}</div>
                  <div class="text-sm text-gray-500">ID: {{ deleteTarget.classId }}</div>
                </div>
              </div>
            </div>
...
import { listClassInfoByPage, deleteClassInfo, addClassInfo, updateClassInfo } from '@/api/classinfo'
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await listClassInfoByPage({
...
const confirmDelete = async () => {
  ...
  deleting.value = true
  try {
    await deleteClassInfo(deleteTarget.value.classId as number)
